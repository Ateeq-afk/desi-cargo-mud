...
BEGIN
  ...
  DROP FUNCTION IF EXISTS check_user_exists(uuid) CASCADE;
...
-- Create new organization policies with unique names
CREATE POLICY "org_select_policy_v1"
  ON organizations
  FOR SELECT
  USING (
    -- Allow super admin
...
CREATE POLICY "org_insert_policy_v1"
  ON organizations
  FOR INSERT
  WITH CHECK (
...
CREATE POLICY "org_update_policy_v1"
  ON organizations
  FOR UPDATE
  USING (
    -- Allow super admin
...
CREATE POLICY "org_delete_policy_v1"
  ON organizations
  FOR DELETE
  USING (
